...
namespace site.sections.project.competitiontheatre
{
    ...
    public partial class reg : System.Web.UI.Page
    {
        ...
        {
            ViewState["TempRequest"] = (CompetitionRequest)Session[EnumsHelper.GetSessionName(Theatre.self)];
        }
        ...
        private void PageFill(string p = "")
        {
            ...
            if (Session[EnumsHelper.GetSessionName(Theatre.self)] == null)
            {
                if (ViewState["TempRequest"] != null)
                {
                    Session[EnumsHelper.GetSessionName(Theatre.self)] = (CompetitionRequest)ViewState["TempRequest"];
                }
                else
                {
                    ViewState["TempRequest"] = new CompetitionRequest();
                    Session[EnumsHelper.GetSessionName(Theatre.self)] = new CompetitionRequest();
                }
            }
            else
            {
            ...
            Page.Title = "Регистрация на " + EnumsHelper.GetCompetitionValueFromCode(EnumsHelper.GetTheatreCode(Theatre.self));
            ...
            if (Request.QueryString["subname"] != null)
            {
                ...
                addPanel.Controls.Add(form.RequestPanel_Theatre(Request.QueryString["subname"]));
            }
        }
