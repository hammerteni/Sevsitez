...
namespace site.sections.project.competitionscience
{
    ...
    public partial class reg : System.Web.UI.Page
    {
        ...
        {
            ViewState["TempRequest"] = (CompetitionRequest)Session[EnumsHelper.GetSessionName(Science.self)];
        }
        ...
        private void PageFill(string p = "")
        {
            ...
            if (Session[EnumsHelper.GetSessionName(Science.self)] == null)
            {
                if (ViewState["TempRequest"] != null)
                {
                    Session[EnumsHelper.GetSessionName(Science.self)] = (CompetitionRequest)ViewState["TempRequest"];
                }
                else
                {
                    ViewState["TempRequest"] = new CompetitionRequest();
                    Session[EnumsHelper.GetSessionName(Science.self)] = new CompetitionRequest();
                }
            }
            else
            {
            ...
            Page.Title = "Регистрация на " + EnumsHelper.GetCompetitionValueFromCode(EnumsHelper.GetScienceCode(Science.self));

            PagesFormE form1 = new PagesFormE(this); p = p.Trim();
            if (p == "") form1.UniverseFillPage(addPanelText, "regscience", PagesFormE.CacheContr.NoCache);
            ...
            if (Request.QueryString["subname"] != null)
            {
                ...
                addPanel.Controls.Add(form.RequestPanel_Science(Request.QueryString["subname"]));
            }
        }
